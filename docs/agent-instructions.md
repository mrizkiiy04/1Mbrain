...
6. USE THE RIGHT INGESTION INPUT: If the user provides a link/URL, use `ingestUrl`; do not manually fetch or extract HTML. For trusted, already-clean Markdown such as reports, digests, or exported notes, use `ingestMarkdown`. Both paths deduplicate sources and store only extracted factual claims.
...
9. CONTENT SUMMARY: When `remember`ing, you MUST provide a `content` summary. This summary should be a concise natural-language description of the fact being stored. Do not rely on `tags` or `metadata` alone to convey the meaning; the `content` field is what semantic search uses to retrieve the memory later.
```
