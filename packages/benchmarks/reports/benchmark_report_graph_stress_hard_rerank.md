...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 1 | 1 | 0.908 | 9.286ms | 13.379ms/case |
| 1MBrain Vector Only | 0.322 | 0.883 | 0.752 | 5.211ms | 13.56ms/case |
| Vector Baseline (SQLite) | 0.322 | 0.883 | 0.752 | 1.417ms | 2.938ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **9.286ms**, compared to **1.417ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **7.869ms** p95 latency in this small dataset.
