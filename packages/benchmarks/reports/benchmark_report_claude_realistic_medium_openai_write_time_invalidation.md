...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 0.722 | 0.897 | 0.726 | 628.195ms | 7192.441ms/case |
| 1MBrain Vector Only | 0.681 | 0.931 | 0.721 | 609.531ms | 7153.238ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **628.195ms**, compared to **0ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **628.195ms** p95 latency in this small dataset.
