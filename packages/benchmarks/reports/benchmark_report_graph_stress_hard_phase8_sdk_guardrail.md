...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 1 | 1 | 0.917 | 4.065ms | 7.079ms/case |
| 1MBrain Vector Only | 0.361 | 0.889 | 0.786 | 1.949ms | 6.009ms/case |
| Vector Baseline (SQLite) | 0.322 | 0.883 | 0.752 | 0.494ms | 1.084ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **4.065ms**, compared to **0.494ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **3.571ms** p95 latency in this small dataset.
