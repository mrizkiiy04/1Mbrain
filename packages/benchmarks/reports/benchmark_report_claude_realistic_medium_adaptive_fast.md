...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 0.628 | 0.782 | 0.576 | 2.379ms | 4.951ms/case |
| 1MBrain Vector Only | 0.619 | 0.757 | 0.557 | 1.815ms | 4.746ms/case |
| Vector Baseline (SQLite) | 0.619 | 0.757 | 0.557 | 0.502ms | 1.354ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **2.379ms**, compared to **0.502ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **1.878ms** p95 latency in this small dataset.
