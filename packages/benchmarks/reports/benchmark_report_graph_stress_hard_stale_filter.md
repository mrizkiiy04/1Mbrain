...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 1 | 1 | 0.908 | 9.924ms | 17.479ms/case |
| 1MBrain Vector Only | 0.322 | 0.883 | 0.767 | 7.921ms | 20.486ms/case |
| Vector Baseline (SQLite) | 0.322 | 0.883 | 0.752 | 1.833ms | 4.575ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **9.924ms**, compared to **1.833ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **8.092ms** p95 latency in this small dataset.
