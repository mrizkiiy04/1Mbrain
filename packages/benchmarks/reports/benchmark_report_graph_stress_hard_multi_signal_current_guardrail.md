...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 1 | 1 | 0.908 | 7.313ms | 10.817ms/case |
| 1MBrain Vector Only | 0.322 | 0.883 | 0.767 | 3.33ms | 10.099ms/case |
| Vector Baseline (SQLite) | 0.322 | 0.883 | 0.752 | 0.948ms | 1.764ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **7.313ms**, compared to **0.948ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **6.365ms** p95 latency in this small dataset.
