...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 0.628 | 0.782 | 0.576 | 2.319ms | 4.949ms/case |
| 1MBrain Vector Only | 0.619 | 0.757 | 0.557 | 1.747ms | 4.663ms/case |
| Vector Baseline (SQLite) | 0.619 | 0.757 | 0.557 | 0.541ms | 1.414ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **2.319ms**, compared to **0.541ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **1.778ms** p95 latency in this small dataset.
