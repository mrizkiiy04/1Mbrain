...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 1 | 1 | 0.908 | 25.116ms | 35.935ms/case |
| 1MBrain Vector Only | 0.322 | 0.883 | 0.752 | 13.265ms | 31.163ms/case |
| Vector Baseline (SQLite) | 0.322 | 0.883 | 0.752 | 2.245ms | 5.222ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **25.116ms**, compared to **2.245ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **22.87ms** p95 latency in this small dataset.
