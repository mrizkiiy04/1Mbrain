...
|---|---:|---:|---:|---:|---:|
| 1MBrain Graph Full | 1 | 1 | 0.908 | 9.618ms | 15.448ms/case |
| 1MBrain Vector Only | 0.322 | 0.883 | 0.767 | 9.576ms | 19.871ms/case |
| Vector Baseline (SQLite) | 0.322 | 0.883 | 0.752 | 2.666ms | 5.125ms/case |
...
The main weakness is not graph traversal cost; it is retrieval precision under paraphrase, stale preference conflicts, and noisy distractors. Graph Full p95 latency is **9.618ms**, compared to **2.666ms** for the raw SQLite vector baseline. This is still low in absolute terms, but quality improvements are modest because the benchmark currently uses a local keyword embedder rather than a stronger semantic embedder.
...
- **Latency:** SQLite vector-only baseline is the fastest, while graph traversal adds roughly **6.952ms** p95 latency in this small dataset.
